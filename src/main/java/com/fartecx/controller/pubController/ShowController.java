...
public class ShowController {
    ...
    @Autowired
    private ShowFarmService showFarmService;



    /**
     * @api {get} api/pub/v1/show
     * @apiGroup show
     * @apiVersion 1.0.0
     * @apiDescription 农场秀列表获取
     ...
     */
    @GetMapping("/show")
    public Result findShow(Integer offset,Integer limit,String keyword){
        try {
            int num = offset == null ? 0: offset;
            int size = limit == null ? 10: limit;
            ExecutionResult executionResult=showFarmService.list(num,size,keyword);
    ...
    /**
     * @api {get} api/pub/v1/show/{id}
     * @apiGroup show
     * @apiVersion 1.0.0
     * @apiDescription 农场秀详情获取
     * @apiParam {String} id
     * @apiSuccess {String} message 访问状态
     * @apiSuccess {int} code 返回码
     * @apiSuccess {List} data 数据信息
    ...
    @GetMapping("/show/{id}")
    public Result findShowById(@PathVariable("id") String id){
        try {
            ExecutionResult executionResult=showFarmService.findShowById(id);
            return new Result(0, "查询成功", executionResult);
        } catch (Exception e) {
            return new Result(1, e.getMessage());
        }

...
public class AnnouController {
    ...
    /**
     * @api {get} api/pub/v1/annou
     * @apiGroup announcement
     * @apiVersion 1.0.0
     * @apiDescription 公布信息列表获取
     ...
     */
    @GetMapping("/annou")
    public Result findAnnouncement(Integer offset,Integer limit,String keyword){
        try {
            int num = offset == null ? 0: offset;
            int size = limit == null ? 10: limit;
            ExecutionResult executionResult=announceService.list(num,size,keyword);
    ...
    /**
     * @api {get} api/pub/v1/annou/{id}
     * @apiGroup announcement
     * @apiVersion 1.0.0
     * @apiDescription 公布信息详情获取
     * @apiParam {String} id
     * @apiSuccess {String} message 访问状态
     * @apiSuccess {int} code 返回码
     * @apiSuccess {List} data 数据信息get
     ...
     */
    @GetMapping("/annou/{id}")
    public Result findAnnouncementById(@PathVariable("id") String id){
        try {
            ExecutionResult executionResult=announceService.findAnnouncementById(id);
            return new Result(0, "查询成功", executionResult);
        } catch (Exception e) {
            return new Result(1, e.getMessage());
        }
